...
RSpec.configure do |config|
  config.define_derived_metadata(file_path: %r{spec/sbmt/pact/}) { |metadata| metadata[:pact] = true }

  # it's not an error: consumer tests contain `providers` subdirectory (because we're testing against different providers)
  config.define_derived_metadata(file_path: %r{spec/sbmt/pact/providers/}) { |metadata| metadata[:pact_entity] = :consumer }
  # for provider tests it's the same thing: we're running tests which test consumers
  config.define_derived_metadata(file_path: %r{spec/sbmt/pact/consumers/}) { |metadata| metadata[:pact_entity] = :provider }
