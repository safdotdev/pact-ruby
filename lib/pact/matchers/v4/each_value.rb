      ...
      module V4
        class EachValue < Pact::Matchers::Base
          def initialize(value_matchers, template)
            ...
            raise MatcherInitializationError, "#{self.class}: #{value_matchers} should be instances of Pact::Matchers::Base" unless value_matchers.all?(Pact::Matchers::Base)
            raise MatcherInitializationError, "#{self.class}: #{value_matchers} size should be greater than 0" unless value_matchers.size > 0

            super(spec_version: Pact::Matchers::PACT_SPEC_V4, kind: "each-value", template: template, opts: {rules: value_matchers})
          end
