      ...
      module V4
        class EachKey < Pact::Matchers::Base
          def initialize(key_matchers, template)
            ...
            raise MatcherInitializationError, "#{self.class}: #{key_matchers} should be instances of Pact::Matchers::Base" unless key_matchers.all?(Pact::Matchers::Base)
            raise MatcherInitializationError, "#{self.class}: #{key_matchers} size should be greater than 0" unless key_matchers.size > 0

            super(spec_version: Pact::Matchers::PACT_SPEC_V4, kind: "each-key", template: template, opts: {rules: key_matchers})
          end
