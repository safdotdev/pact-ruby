...
require 'pact/consumer/interaction_builder_ffi'
require 'pact/ffi'
  ...
  module Consumer
    module Ffi
      class ConsumerContractBuilder
        ...
        attr_reader :mock_service_base_url, :port
        ...
        def initialize(attributes)
          ...
          puts attributes
          PactFfi::HttpConsumer.with_specification(@pact, attributes[:pact_specification_version].to_i + 1)
        end
        ...
        def start_mock
          @port = PactFfi::MockServer.create_for_transport(@pact, @mock_server_host, @port,
                                                          'http', nil)
           @mock_service_base_url = "http://" + @mock_server_host + ":" + @port.to_s
        end
        ...
        def interaction_builder
          @interaction_builder ||=
          begin
            interaction_builder = Pact::Consumer::Ffi::InteractionBuilder.new(@pact) do |interaction|
