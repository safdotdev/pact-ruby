  ...
  module Cli
    module Ffi
      class RunPactVerification
        ...
        def setup_load_path
          require 'pact/provider/pact_spec_runner_ffi'
        ...
        def run_with_pact_url_string
          ...
          Pact::Provider::Ffi::PactSpecRunner.new([pact_uri], pact_spec_options).run
        end

        def run_with_pact_uri_object
          Pact::Provider::Ffi::PactSpecRunner.new([options[:pact_uri]], pact_spec_options).run
        end

        def run_with_configured_pacts_from_pact_helper
          pact_urls = Pact.provider_world.pact_urls
          Pact::Provider::Ffi::PactSpecRunner.new(pact_urls, pact_spec_options).run
        end
