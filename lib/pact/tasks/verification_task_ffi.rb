...
module Pact
  module Ffi
    class VerificationTask < ::Rake::TaskLib
      ...
      def rake_task
        namespace :pact_ffi do

          desc "Verify provider against the consumer pacts for #{name}"
          task "verify:#{name}" do |t, args|

            require 'pact/tasks/task_helper_ffi'

            exit_statuses = pact_spec_configs.collect do | config |
              Pact::Ffi::TaskHelper.execute_pact_verify config[:uri], config[:pact_helper], config[:provider_base_url], rspec_opts, { ignore_failures: ignore_failures }
            end

            Pact::Ffi::TaskHelper.handle_verification_failure do
